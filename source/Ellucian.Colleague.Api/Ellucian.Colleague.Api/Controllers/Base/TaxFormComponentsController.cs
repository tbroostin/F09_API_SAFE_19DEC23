...
namespace Ellucian.Colleague.Api.Controllers.Base
{
    ...
    public class TaxFormComponentsController : BaseCompressedApiController
    {
        ...
        public TaxFormComponentsController(ITaxFormComponentsService taxFormComponentsService, ILogger logger)
        {
        ...
        public async Task<IEnumerable<Ellucian.Colleague.Dtos.TaxFormComponents>> GetTaxFormComponentsAsync()
        {
        ...
        [CustomMediaTypeAttributeFilter(ErrorContentType = IntegrationErrors2)]
        [HttpGet, EedmResponseFilter]
        public async Task<Dtos.TaxFormComponents> GetTaxFormComponentsByGuidAsync(string guid)
        {
            var bypassCache = false;
            if (Request.Headers.CacheControl != null)
            {
        ...
        /// <returns>A taxFormComponents object <see cref="Dtos.TaxFormComponents"/> in EEDM format</returns>
        [HttpPost]
        public async Task<Dtos.TaxFormComponents> PostTaxFormComponentsAsync([FromBody] Dtos.TaxFormComponents taxFormComponents)
        {
        ...
        /// <returns>A taxFormComponents object <see cref="Dtos.TaxFormComponents"/> in EEDM format</returns>
        [HttpPut]
        public async Task<Dtos.TaxFormComponents> PutTaxFormComponentsAsync([FromUri] string guid, [FromBody] Dtos.TaxFormComponents taxFormComponents)
        {
            //Update is not supported for Colleague but HeDM requires full crud support.
            throw CreateHttpResponseException(new IntegrationApiException(IntegrationApiUtility.DefaultNotSupportedApiErrorMessage, IntegrationApiUtility.DefaultNotSupportedApiError));
        }
        ...
        /// <param name="guid">GUID to desired taxFormComponents</param>
        [HttpDelete]
        public async Task DeleteTaxFormComponentsAsync(string guid)
        {
            //Update is not supported for Colleague but HeDM requires full crud support.
            throw CreateHttpResponseException(new IntegrationApiException(IntegrationApiUtility.DefaultNotSupportedApiErrorMessage, IntegrationApiUtility.DefaultNotSupportedApiError));
        }

...
namespace Ellucian.Colleague.Api.Controllers.Student
{
    ...
    public class StudentPaymentsController : BaseCompressedApiController
    {
        ...
        [CustomMediaTypeAttributeFilter(ErrorContentType = IntegrationErrors2)]
        [HttpGet, EedmResponseFilter]
        public async Task<Dtos.StudentPayment3> GetByIdAsync3([FromUri] string id)
        {
            bool bypassCache = false;
            if (Request.Headers.CacheControl != null)
            {
        ...
        [HttpGet]
        [ValidateQueryStringFilter()]
        [QueryStringFilterFilter("criteria", typeof(Dtos.StudentPayment3)), FilteringFilter(IgnoreFiltering = true)]
        [PagingFilter(IgnorePaging = true, DefaultLimit = 100), EedmResponseFilter]
        public async Task<IHttpActionResult> GetAsync3(Paging page, QueryStringFilter criteria)
        {
            string student = "", academicPeriod = "", fundSource = "", fundDestination = "", paymentType = "", usage = "";
            ...
            try
            {
                ...
                if (CheckForEmptyFilterParameters())
                    return new PagedHttpActionResult<IEnumerable<Dtos.StudentPayment3>>(new List<Dtos.StudentPayment3>(), page, 0, this.Request);
                ...
                return new PagedHttpActionResult<IEnumerable<Dtos.StudentPayment3>>(pageOfItems.Item1, page, pageOfItems.Item2, this.Request);
        ...
        [CustomMediaTypeAttributeFilter(ErrorContentType = IntegrationErrors2)]
        [HttpPost, EedmResponseFilter]
        public async Task<Dtos.StudentPayment3> CreateAsync3([ModelBinder(typeof(EedmModelBinder))] Dtos.StudentPayment3 studentPaymentDto)
        {
            try
            {
                if (studentPaymentDto == null)

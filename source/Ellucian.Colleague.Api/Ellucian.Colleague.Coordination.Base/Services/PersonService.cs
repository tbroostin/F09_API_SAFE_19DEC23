...
namespace Ellucian.Colleague.Coordination.Base.Services
{
    ...
    public class PersonService : BaseCoordinationService, IPersonService
    {
        ...
        public async Task<Tuple<IEnumerable<Dtos.PersonCredential2>, int>> GetAllPersonCredentials2Async(int offset, int limit, bool bypassCache)
        {
            try
            {
                var personGuids = await _personRepository.GetPersonGuidsAsync(offset, limit, bypassCache);
                if (personGuids == null || personGuids.Item1 == null)
                {
                    return new Tuple<IEnumerable<Dtos.PersonCredential2>, int>(new List<PersonCredential2>(), 0);
                }

                var personEntities = await _personRepository.GetPersonIntegrationByGuidNonCachedAsync(personGuids.Item1);
                var personIds = personEntities
                        .Where(x => (!string.IsNullOrEmpty(x.Id)))
                    .Select(x => x.Id).Distinct().ToList();
                await this.GetPersonUserNames(personIds.ToArray());
                ...
                return personCredentials.Any() ? new Tuple<IEnumerable<Dtos.PersonCredential2>, int>(personCredentials, personGuids.Item2) :
        ...
        {
            //CheckUserPersonCredentialsViewPermissions();
            ...
            try
            {
                ...
                if (personFilterCredentials != null && personFilterCredentials.Credentials != null && personFilterCredentials.Credentials.Any())
                {
                    List<Tuple<string, string>> creds = new List<Tuple<string, string>>();
                    personFilterCredentials.Credentials.ToList().ForEach(i =>
                    {
                        var tempValue = !string.IsNullOrWhiteSpace(i.Value) && i.Value.Contains("'") ? i.Value.Replace("'", string.Empty) : i.Value;
                        Tuple<string, string> tuple = new Tuple<string, string>(i.Type.ToString(), tempValue);
                        creds.Add(tuple);
                    });
                    personFilterCriteria.Credentials = creds;
                ...
                if (personFilterCredentials != null && personFilterCredentials.AlternativeCredentials != null && personFilterCredentials.AlternativeCredentials.Any())
                {
                    var altIdTypesList = await GetAlternateIdTypesAsync(false);
                    List<Tuple<string, string>> altCreds = new List<Tuple<string, string>>();

                    //Make sure altIdTypesList is not null
                    if (altIdTypesList != null && altIdTypesList.Any())
                    {
                        personFilterCredentials.AlternativeCredentials.ToList().ForEach(i =>
                        {
                            if (i.Type != null && !string.IsNullOrEmpty(i.Type.Id))
                            {
                                var altCredTypeEntity = altIdTypesList.FirstOrDefault(e => !string.IsNullOrEmpty(e.Guid) && e.Guid.Equals(i.Type.Id, StringComparison.OrdinalIgnoreCase));
                    ...
                    {
                        //Per Vickie, return an empty set
                        return new Tuple<IEnumerable<Dtos.PersonCredential3>, int>(new List<PersonCredential3>(), 0);
                    }
                }
                ...
                var personGuids = await _personRepository.GetFilteredPerson2GuidsAsync(offset, limit, bypassCache, personFilterCriteria, null);

                var personCredentials = new List<PersonCredential3>();
                if (personGuids != null && personGuids.Item1 != null && personGuids.Item1.Any())
                {

                    var personEntities = await _personRepository.GetPersonCredentialsIntegrationByGuidNonCachedAsync(personGuids.Item1);
                    var personIds = personEntities
                        .Where(x => (!string.IsNullOrEmpty(x.Id)))
                    .Select(x => x.Id).Distinct().ToList();
                    await this.GetPersonUserNames(personIds.ToArray());
                ...
                return personCredentials.Any() ? new Tuple<IEnumerable<Dtos.PersonCredential3>, int>(personCredentials, personGuids.Item2) :
        ...
        public async Task<Dtos.PersonCredential2> GetPersonCredential2ByGuidAsync(string id)
        {
            ...
            await this.GetPersonUserNames(new string[] { personEntity.Id });
        ...
        public async Task<Dtos.PersonCredential3> GetPersonCredential4ByGuidAsync(string id)
        {
            ...
            try
            {
                personEntity = await _personRepository.GetPersonCredentialByGuidNonCachedAsync(id);
            }
            catch (ArgumentNullException e)
            {
                IntegrationApiExceptionAddError(string.Format("No person credentials was found for guid '{0}'.", id), "GUID.Not.Found", id, string.Empty, System.Net.HttpStatusCode.NotFound);
            ...
            Domain.Base.Entities.Person person)
        {
            ...
            // ORG.ENTITY.ENV person user names
            if (_personUserNames != null && _personUserNames.Any())
            {
                var personUserNameEntity = _personUserNames.FirstOrDefault(i => !string.IsNullOrEmpty(i.PersonUserNameId) && i.PersonUserNameId.Equals(person.Id, StringComparison.OrdinalIgnoreCase));
                if ((personUserNameEntity != null) && (!string.IsNullOrEmpty(personUserNameEntity.UserName)))
                {
                    credentials.Add(new Dtos.DtoProperties.CredentialDtoProperty2()
                    {
                        Type = Dtos.EnumProperties.CredentialType2.ColleagueUserName,
                        Value = personUserNameEntity.UserName
                    });
                }
            }
            return credentials;
        ...
        /// <summary>
        /// Get all persons' user names
        /// </summary>
        public IEnumerable<PersonUserName> _personUserNames { get; set; }
        private async Task<IEnumerable<PersonUserName>> GetPersonUserNames(string[] ids)
        {
            if ((_personUserNames == null) && (ids != null) && (ids.Any()))
            {
                _personUserNames = await _personRepository.GetPersonUserNamesAsync(ids);
            }
            return _personUserNames;
        }
        ...
        public async Task<Dtos.Base.Profile> GetProfileAsync(string personId, bool useCache = true)
        {
            try
            {
                ...
                {
                    throw new ColleagueWebApiException("Profile information could not be retrieved for person " + personId);
                }
        ...
        public async Task<Dtos.Base.PersonProxyDetails> GetPersonProxyDetailsAsync(string personId)
        {
            ...
            if (!await UserCanViewProfileForPerson(personId) && !HasPermission(BasePermissionCodes.AddAllEmployeeProxy))
            {
                string message = CurrentUser + " cannot view profile for person " + personId;
                logger.Error(message);
                throw new PermissionsException();
            ...
            if (person == null)
            {
                throw new ColleagueWebApiException("Information could not be retrieved for person " + personId);
            }
            if (person.IsDeceased)
            {
                throw new ColleagueWebApiException("Cannot retrieve proxy details for a deceased person");
            }
            ...
            if (proxyConfig == null)
            {
                throw new ColleagueWebApiException("Could not retrieve proxy configuration");
            }
            ...
            Domain.Base.Entities.PersonIntegration person, bool bypassCache = false)
        {
            ...
            if (personNameTypeItem != null)
            {
                ...
                var lastNamePrefix =
                    person.LastName.TrimStart().Contains(" ")
                    ? person.LastName.TrimStart().Split(" ".ToCharArray())[0].ToString()
                    : null;
                if (!string.IsNullOrEmpty(lastNamePrefix))
                {
                    personName.LastNamePrefix = lastNamePrefix;
                }
                ...
                !string.IsNullOrEmpty(person.BirthNameMiddle))
            {
                ...
                if (birthNameTypeItem != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(lastNamePrefix))
                    {
                        birthName.LastNamePrefix = lastNamePrefix;
                    }
                ...
                !string.IsNullOrEmpty(person.ChosenMiddleName))
            {
                ...
                if (chosenNameTypeItem != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(lastNamePrefix))
                    {
                        chosenName.LastNamePrefix = lastNamePrefix;
                    }
            ...
            if ((person.FormerNames != null) && (person.FormerNames.Any()))
            {
                ...
                if (historyNameTypeItem != null)
                {
                    foreach (var name in person.FormerNames)
                    {
                        ...
                        if (!string.IsNullOrEmpty(lastNamePrefix))
                        {
                            formerName.LastNamePrefix = lastNamePrefix;
                        }
        ...
        private async Task<IEnumerable<AlternativeCredentials>> GetAlternativeCredentials(string personId, List<PersonAlt> personAltIds, bool bypassCache)
        {
            ...
            if (personAltIds != null && personAltIds.Any())
            {
                ...
                var altCredTypes = await GetAlternateIdTypesAsync(bypassCache);
                ...
                foreach (var cred in personAltIds)
                {
                    ...
                    if (cred != null && cred.Type != Domain.Base.Entities.PersonAlt.ElevatePersonAltType)
                    {
                        if (!string.IsNullOrEmpty(cred.Type))
                        {
                            try
                            {
                                if (altCredTypes != null && altCredTypes.Any())
                                {
                                    var credType = ConvertCodeToGuid(altCredTypes, cred.Type);
            ...
            Domain.Base.Entities.PersonIntegration personEntity, bool getLimitedData, bool bypassCache = false)
        {
            ...
            personDto.Id = personEntity.Guid;
            var privacyStatus = await GetPersonPrivacyAsync(personEntity.PrivacyStatus, personEntity.PrivacyStatusCode, bypassCache);
            ...
            var religion = (!string.IsNullOrEmpty(personEntity.Religion)
                ? await GetPersonReligionGuidAsync(personEntity.Religion)
                : null);
            if (religion != null)
            {
                personDto.Religion = religion;
            }
            var ethnicity = (personEntity.EthnicCodes != null && personEntity.EthnicCodes.Any()
                ? await GetPersonEthnicityGuidAsync(personEntity.EthnicCodes)
                : null);
            if (ethnicity != null)
            {
                personDto.Ethnicity = ethnicity;
            }
            var races = (personEntity.RaceCodes != null && personEntity.RaceCodes.Any()
                ? await GetPersonRaceGuidsAsync(personEntity.RaceCodes)
                : null);
            if (races != null)
            {
                personDto.Races = races;
            }
            if ((personEntity.PrimaryLanguage != null) || (personEntity.SecondaryLanguages != null && personEntity.SecondaryLanguages.Any()))
            {
            ...
            if (!string.IsNullOrEmpty(personEntity.MaritalStatusCode))
            {
                var personMaritalStatus = await GetPersonMaritalStatus2GuidAsync(personEntity.MaritalStatusCode);
                if (personMaritalStatus != null)
                {
                    personDto.MaritalStatus = personMaritalStatus;
                }
            }
            if (personEntity.AlienStatus != null)
            {
                var citizenshipStatus = await GetPersonCitizenshipAsync(personEntity.AlienStatus, bypassCache);
            ...
            string[] personIds = { personEntity.Id };
            await this.GetPersonUserNames(personIds);
            ...
            if ((personEntity.Interests != null) && (personEntity.Interests.Any()))
            {
                var interests = await GetPersonInterestsAsync(personEntity.Interests, bypassCache);
                if (interests != null)
                {
                    personDto.Interests = interests;
                }
            }
            ...
            var phoneNumbers = await GetPhones3Async(phoneEntities, bypassCache);
            if ((phoneNumbers != null) && (phoneNumbers.Any())) personDto.Phones = phoneNumbers;
            if ((socialMediaEntities != null) && (socialMediaEntities.Any()))
                personDto.SocialMedia = await GetPersonSocialMediaAsync(socialMediaEntities, bypassCache);
        ...
        public async Task<Tuple<IEnumerable<Dtos.Person5>, int>> GetPerson5NonCachedAsync(int offset, int limit, bool bypassCache, Person5 person, string personFilter)
        {
            ...
            if (person != null)
            {
                if (person.Credentials != null && person.Credentials.Any())
                {
                    ...
                    foreach (var i in person.Credentials)
                    {
                        var tempValue = !string.IsNullOrWhiteSpace(i.Value) && i.Value.Contains("'") ? i.Value.Replace("'", string.Empty) : i.Value;
                        var type = i.Type;
                        Tuple<string, string> tuple = null;
                        if (type == Credential3Type.ColleaguePersonId && !string.IsNullOrEmpty(tempValue))
                        {
                            var isCorp = await _personRepository.IsCorpAsync(tempValue);
                            if (!isCorp)
                                tuple = new Tuple<string, string>(i.Type.ToString(), tempValue);
                        }
                        else
                        {
                            tuple = new Tuple<string, string>(i.Type.ToString(), tempValue);
        ...
        public async Task<Dtos.Person2> CreatePerson2Async(Dtos.Person2 personDto)
        {
            ...
            try
            {
                ...
                ValidateAddressDataAsync(personDto.Addresses, await GetCountiesAsync(false));
        ...
        public async Task<Dtos.Person5> CreatePerson5Async(Dtos.Person5 personDto)
        {
            ...
            try
            {
                ...
                try
                {
                    phoneEntities = await ConvertPhoneDtoToPhoneEntities2(personDto.Phones);
                }
                catch (Exception ex)
                {
            ...
            catch (IntegrationApiException iaex)
            {
                // Drop through on Integration API exception
                logger.Error(iaex.Message, "Error creating person.");
            }
            catch (RepositoryException ex)
            {
        ...
        public async Task<Dtos.Person2> UpdatePerson2Async(Dtos.Person2 personDto)
        {
            ...
            catch (KeyNotFoundException knfex)
            {
                ...
                logger.Error(knfex.Message, "Error getting person ID");
            }
            _personRepository.EthosExtendedDataDictionary = EthosExtendedDataDictionary;
        ...
        public async Task<Dtos.Person5> UpdatePerson5Async(Dtos.Person5 personDto)
        {
            ...
            if (!string.IsNullOrEmpty(personId))
            {
                ...
                {
                    // Drop through on Integration API exception
                    logger.Error(iaex.Message, "Error updating person.");
                }
                catch (RepositoryException ex)
                {
                    IntegrationApiExceptionAddError(ex, guid: personGuid, id: personId);
            ...
            string role, string credentialType, string credentialValue)
        {
            ...
            try
            {
                filteredTuple = await _personRepository.GetFilteredOrganizationGuidsAsync(offset, limit,
            ...
            if (filteredTuple.Item1 != null && filteredTuple.Item1.Any())
            {
                ...
                foreach (var personEntity in personEntities)
                {
                    Dtos.Organization2 orgDto = null;
                    orgDto = await ConvertPersonIntegrationEntityToOrganizationDtoAsync(personEntity);
                    if (orgDto != null)
                    {
                        orgDtos.Add(orgDto);
                    }
        ...
        {
            Dtos.Organization2 organizationDto;

            // get the person ID associated with the incoming guid
            string personId = string.Empty;
            Ellucian.Colleague.Domain.Base.Entities.Person personOrg = null;
            PersonIntegration personIntgData = null;
            try
            {
                personId = await _personRepository.GetPersonIdFromGuidAsync(guid);
            }
            ...
            {
                throw new KeyNotFoundException(string.Format("No organizations was found for guid '{0}'.", guid));
            }
            try
            {
                personOrg = await _personRepository.GetPersonByGuidNonCachedAsync(guid);
            }
            catch (Exception ex)
            {
                IntegrationApiExceptionAddError(ex.Message, "Bad.Data",
            ...
            else
            {
                ...
                {
                    IntegrationApiExceptionAddError(string.Format("The id {0} does not belong to an organization, it belongs to a person.", guid), "Bad.Data", personOrg.Guid, personOrg.Id);
                    throw IntegrationApiException;
                }
            }
            try
            {
                personIntgData = await _personRepository.GetPersonIntegrationByGuidNonCachedAsync(guid);
            }
            catch (Exception ex)
            {
                IntegrationApiExceptionAddError(ex.Message, "Bad.Data",
                     personOrg.Guid, personOrg.Id);
                throw IntegrationApiException;

            }

            organizationDto = await ConvertPersonIntegrationEntityToOrganizationDtoAsync(personIntgData);
        ...
        public async Task<Organization2> UpdateOrganizationAsync(Organization2 organizationDto)
        {
            ...
            try
            {
                ...
                string personOrgId = null;
                try
                {
                    // get the person ID associated with the incoming guid
                    personOrgId = await _personRepository.GetPersonIdFromGuidAsync(organizationDto.Id);
                }
                catch (KeyNotFoundException knfex)
                {
                    // suppress KeyNotFound.  PUT can be called with an externally-supplied GUID to create a new person
        ...
        public async Task<IEnumerable<Dtos.Person2>> QueryPerson2ByPostAsync(Dtos.Person2 personDtoIn)
        {
            ...
            PersonNameDtoProperty personPrimaryName = null;
            try
            {
            ...
            PersonNameDtoProperty personBirthName = null;
            try
            {
                ...
                if (personBirthName != null && personPrimaryName == null)
                {
                    personPrimaryName = personBirthName;
                }
            }
            catch
            {
            ...
            if (personBirthName != null && (string.IsNullOrEmpty(personBirthName.FirstName) ||
                string.IsNullOrEmpty(personBirthName.LastName)))
            {
                throw new ArgumentNullException("personDtoIn", "Person first and last name required to query");
            }
            if (personPrimaryName == null && personBirthName == null)
            {
                throw new ArgumentNullException("personDtoIn", "Person name type category must be 'legal' or 'birth' to query.");
            }
        ...
        public async Task<IEnumerable<Dtos.Person4>> QueryPerson4ByPostAsync(Dtos.Person4 personDtoIn, bool bypassCache = false)
        {
            ...
            {
                // Leave personBirthName null if we can't find a match.
                logger.Error(ex.Message, "Cannot match person birth name.");
            }
            if (personBirthName != null && (string.IsNullOrEmpty(personBirthName.FirstName) ||
                string.IsNullOrEmpty(personBirthName.LastName)))
            {
        ...
        public async Task<IEnumerable<Dtos.Person5>> QueryPerson5ByPostAsync(Dtos.Person5 personDtoIn, bool bypassCache = false)
        {
            if (personDtoIn == null)
            {
                IntegrationApiExceptionAddError("Person required to query.", "Validation.Exception");
                throw IntegrationApiException;
            }
            if (personDtoIn.PersonNames == null)
            {
                IntegrationApiExceptionAddError("Names must be defined in query.", "Validation.Exception");
                throw IntegrationApiException;
            }
            ...
            {
                var code = "Validation.Exception";
                IntegrationApiExceptionAddError("Person name type category must be 'legal' or 'birth' for matching.", code);
                throw IntegrationApiException;
            }
        ...
        /// Logged in user with the permission ADD.ALL.HR.PROXY, can view the profile of anyone. 
        /// </summary
        private async Task<bool> UserCanViewProfileForPerson(string personId)
        {
            if (!CurrentUser.IsPerson(personId) && !HasProxyAccessForPerson(personId) && !HasPermission(BasePermissionCodes.AddAllEmployeeProxy))
            {
        ...
        //
        // With v12+, we no longer default spec processing 1 for MARITAL.STATUSES = "7" ('CO'rrection) or null as "single".  The are both now "other".
        //
        private async Task<Dtos.DtoProperties.PersonMaritalStatusDtoProperty> GetPersonMaritalStatus2GuidAsync(
            string maritalStatusCode, bool bypassCache = false)
        {
            // get the marital status guid and category
            if (!string.IsNullOrEmpty(maritalStatusCode))
                ...
                new Dtos.DtoProperties.PersonMaritalStatusDtoProperty();
                    ...
                    switch (maritalStatusEntity.Type)
                    {
                        ...
                        case (Ellucian.Colleague.Domain.Base.Entities.MaritalStatusType.Widowed):
                            maritalStatusDto.MaritalCategory = Dtos.EnumProperties.PersonMaritalStatusCategory.Widowed;
                            break;
                        case (Ellucian.Colleague.Domain.Base.Entities.MaritalStatusType.Other):
                            maritalStatusDto.MaritalCategory = Dtos.EnumProperties.PersonMaritalStatusCategory.Other;
                            break;
                        default:
                            maritalStatusDto.MaritalCategory = Dtos.EnumProperties.PersonMaritalStatusCategory.Other;
                            break;
        ...
        {
            List<Dtos.DtoProperties.PersonRaceDtoProperty> raceDtos = null;
            ...
            if (raceCodes != null && raceCodes.Count() > 0)
            {
                foreach (var raceCode in raceCodes)
                {
                    ...
                    if (raceEntity != null && !string.IsNullOrEmpty(raceEntity.Guid))
                    {
                        ...
                        if (raceDtos == null)
                        {
                            raceDtos = new List<Dtos.DtoProperties.PersonRaceDtoProperty>();
                        }
        ...
        {
            Dtos.GuidObject2 religionDto = null;
            ...
            if (!string.IsNullOrEmpty(religionCode))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger.Error(ex.Message, "religion code not found in denominations table");
                }
            }
            return religionDto;
        }
        ...
        {
            Dtos.DtoProperties.PersonEthnicityDtoProperty ethnicityDto = null;
            // get the ethnicity guid and category
            if (ethnicityCodes != null && ethnicityCodes.Count() > 0)
            {
                ...
                {
                    ethnicityDto = new Dtos.DtoProperties.PersonEthnicityDtoProperty();
                    ethnicityDto.EthnicGroup = new Dtos.GuidObject2(ethnicityEntity.Guid);
                    if (ethnicityEntity.Type != null)
                    {
                        ...
                        switch (ethnicityEntity.Type)
                        {
                            case EthnicityType.Hispanic:
                                ...
                                break;
                            case EthnicityType.NonHispanic:
                                ethnicityReportingItem.Country.EthnicCategory =
        ...
        private async Task<IEnumerable<Dtos.DtoProperties.PersonAddressDtoProperty>> GetOrganizationAddressesAsync(
          IEnumerable<Domain.Base.Entities.Address> addressEntities, string personGuid, string personId, bool bypassCache = false)
        {
                ...
                foreach (var addressEntity in addressEntities)
                {
                    if (addressEntity != null && addressEntity.TypeCode != null && !string.IsNullOrEmpty(addressEntity.TypeCode))
                    {
                        ...
                        for (int i = 0; i < addrTypes.Length; i++)
                        {
                            ...
                            {
                                IntegrationApiExceptionAddError(string.Format("Unable to find the GUID for the address id '{0}'", addressEntity.AddressId), "GUID.Not.Found", personGuid, personId);
                            }
                            if (addressTypes != null && addressTypes.Any())
                            {
                                ...
                                if (type != null)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(type.Guid))
                                        ...
                                        IntegrationApiExceptionAddError(string.Format("Unable to find the GUID for the address type '{0}'", addrType), "GUID.Not.Found");
                                ...
                                else
                                {
                                    ...
                                    try
                                    {
                                        addressId = await _personRepository.GetAddressIdFromGuidAsync(addressEntity.Guid);
                                    }
                                    catch (Exception ex)
                                    {
                                        logger.Error(ex.Message, "Cannot address ID from guid");
                                    }
                                    IntegrationApiExceptionAddError(string.Format("Unable to find the GUID for the address type '{0}' for address record ID '{1}'", addrType, addressId), "GUID.Not.Found", personGuid, personId);
                                }
                            }
                            else
                            {
                            ...
                            if (addressEntity.IsPreferredAddress && i == 0)
                            ...
                            if (addressEntity.SeasonalDates != null)
                            {
                                ...
                                foreach (var assocEntity in addressEntity.SeasonalDates)
                                {
                                    try
                                    {
                                        ...
                                        if (endMonth == startMonth && endDay < startDay) endYear = endYear + 1;
                                        ...
                                        recurrence.RepeatRule = new Dtos.RepeatRuleDaily()
                                        {
                                            ...
                                            Interval = 1,
                                            Ends = new Dtos.RepeatRuleEnds() { Date = new DateTime(endYear, endMonth, endDay) }
                                        };
                                    ...
                                    catch (Exception ex)
                                    {
                                        ...
                                        logger.Error(ex.Message, "Invalid seasonal start or end date");
                                    }
                                }
                    ...
                    //as per BA, do not throw error for missing addrss type.It is okay for the address to not display.
                    //else
                    //address type is required and missing it creates a bad data situation
                    //{
                    //IntegrationApiExceptionAddError(string.Format("The address id '{0}' does not have an address type.", addressEntity.AddressId), "Bad.Data", personGuid, personId);
                    //}
                }
            }
            return addressDtos;
            ...
            if (phoneEntities != null && phoneEntities.Count() > 0)
            {
                foreach (var phoneEntity in phoneEntities)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        logger.Error(ex.Message, "Cannot find guid. Phone number excluded from output.");
                    }
                }
            }
            return phoneDtos;
        ...
        private async Task<IEnumerable<Dtos.DtoProperties.PersonPhone2DtoProperty>> GetPhones3Async(
            IEnumerable<Domain.Base.Entities.Phone> phoneEntities, bool bypassCache = false)
        {
            var phoneDtos = new List<Dtos.DtoProperties.PersonPhone2DtoProperty>();
            if (phoneEntities != null && phoneEntities.Count() > 0)
            {
                foreach (var phoneEntity in phoneEntities)
                {
                    ...
                    try
                    {
                        ...
                        var phoneDto = new Dtos.DtoProperties.PersonPhone2DtoProperty()
                        {
                            Number = phoneEntity.Number,
                            Extension = string.IsNullOrEmpty(phoneEntity.Extension) ? null : phoneEntity.Extension,
                            Type = new Dtos.DtoProperties.PersonPhoneTypeDtoProperty()
            ...
            IEnumerable<Domain.Base.Entities.EmailAddress> emailAddressEntities, bool bypassCache = false)
        {
            ...
            if (emailAddressEntities != null && emailAddressEntities.Count() > 0)
            {
                foreach (var emailAddressEntity in emailAddressEntities)
                {
                    ...
                    {
                        // do not fail if we can't find a guid from the code table or category
                        // Just exclude the email address from the output.
                        logger.Error(ex.Message, "Cannot find guid. Email address excluded from output.");
                    }
                }
            }
        ...
        {
            Dtos.DtoProperties.PersonPrivacyDtoProperty personPrivacy = null;
            try
            {
                if (string.IsNullOrEmpty(privacyStatusCode))
                {
                    ...
                    {
                        personPrivacy = new PersonPrivacyDtoProperty();
            ...
            { // Do nothing if the code doesn't exist.                        
                logger.Error(ex.Message, "Cannot find privacy status type");
            }
        ...
        {
            Dtos.DtoProperties.PersonCitizenshipDtoProperty citizenshipStatus = null;
            ...
            if (!string.IsNullOrEmpty(alienStatusCode))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger.Error(ex.Message, "Cannot find guid for citizenship status.");
                }
            }
            else
            {
        ...
        private async Task<Dtos.DtoProperties.PersonVisaDtoProperty> GetPersonVisaStatusAsync(PersonVisa personVisaEntity, bool bypassCache = false)
        {
            ...
            if (personVisaEntity != null)
            {
                ...
                catch (Exception ex)
                {
                    // do not fail if we can't find a guid from the code table or category
                    // just return an empty status object
                    logger.Error(ex.Message, "Cannot find guid for visa.");
                }
            }
        ...
        private async Task<IEnumerable<Dtos.DtoProperties.PersonIdentityDocument>> GetPersonIdentityDocumentsAsync(PersonIntegration personEntity, bool bypassCache = false)
        {
            ...
            if (passportEntity != null)
            {
                if (!string.IsNullOrEmpty(passportEntity.PassportNumber))
                {
                    ...
                    {
                        // do not fail if we can't find a guid from the code table or category
                        logger.Error(ex.Message, "Cannot find guid for passport.");
                    }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        logger.Error(ex.Message, "Cannot find identity document");
                    }
                }
            }
            ...
            if (driverEntity != null)
            {
                if (!string.IsNullOrEmpty(driverEntity.LicenseNumber))
                {
                    ...
                    {
                        // do not fail if we can't find a guid from the code table or category
                        logger.Error(ex.Message, "Cannot find guid for drivers license.");
                    }
            ...
            if (personEntity.IdentityDocuments != null)
            {
                foreach (var document in personEntity.IdentityDocuments)
                {
                    if (!string.IsNullOrEmpty(document.Number))
                    {
                        ...
                        {
                            // do not fail if we can't find a guid from the code table or category
                            logger.Error(ex.Message, "Cannot find guid for other identity document.");
                        }
                        ...
                        {
                            // Do not include identity document
                            logger.Error(ex.Message, "Cannot add identity document.");
                        }
                    }
                }
            }
        ...
        {
            List<Dtos.GuidObject2> interestGuids = null;
            ...
            foreach (var interest in interestCodes)
            {
                try
                {
                    var firstOrDefault = (await GetInterestsAsync(bypassCache)).FirstOrDefault(
                        ic => ic.Code == interest);
                    if (firstOrDefault != null && (!string.IsNullOrEmpty(firstOrDefault.Guid)))
                    {
                        var guid = firstOrDefault.Guid;
                        if (interestGuids == null)
                        {
                            interestGuids = new List<Dtos.GuidObject2>();
                        }
                ...
                {
                    // do not fail if we can't find a guid from the code table or category
                    logger.Error(ex.Message, "Cannot find guid for interest.");
                }
            }
            ...
            List<Ellucian.Colleague.Domain.Base.Entities.SocialMedia> mediaTypes, bool bypassCache = false)
        {
            ...
            foreach (var mediaType in mediaTypes)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger.Error(ex.Message, "Cannot find category for social media code.");
                }
            }
            ...
            Dtos.Person3 personDto)
        {
            ...
            if (personDto.Credentials != null && personDto.Credentials.Any())
            {
                if (personDto.Credentials.Count(c => c.Type == Dtos.EnumProperties.CredentialType2.ColleagueUserName) > 0 && !string.IsNullOrEmpty(personId))
                {
                    ...
                    if (personEntity != null)
                    {
                        var thisPerson = new string[] { personId };
                        await this.GetPersonUserNames(thisPerson);
                        if (_personUserNames == null)
                        {
                        ...
                        {
                            var personUserName = _personUserNames.First();
                            if (personUserName == null || personUserName.UserName != username.Value)
                            {
            ...
            Dtos.Person5 personDto)
        {
            ...
            if (personDto.MaritalStatus != null)
            {
                try
                {
                    personEntity = await ConvertPerson2MaritalStatus2DtoToEntityAsync(personEntity, personDto.MaritalStatus);
                }
                catch (Exception ex)
                {
        ...
        private async Task<PersonIntegration> ConvertPersonNames(string personId, string guid, IEnumerable<Dtos.DtoProperties.PersonNameDtoProperty> PersonNames)
        {
            ...
            if (!string.IsNullOrEmpty(primaryName.LastNamePrefix) && !string.IsNullOrEmpty(primaryName.LastName))
            {
                ...
                {
                    if (!string.IsNullOrEmpty(primaryName.LastNamePrefix) && !primaryName.LastNamePrefix.Equals(primaryName.LastName, StringComparison.OrdinalIgnoreCase))
                    {
                        primaryName.LastName = string.Concat(primaryName.LastNamePrefix, " ", primaryName.LastName);
                    }
                }
            ...
            if (birthNames.Any())
            {
                ...
                if (birthName != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(birthName.LastNamePrefix) && !string.IsNullOrEmpty(birthName.LastName))
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(birthName.LastNamePrefix) && !birthName.LastNamePrefix.Equals(birthName.LastName, StringComparison.OrdinalIgnoreCase))
                            {
                                birthName.LastName = string.Concat(birthName.LastNamePrefix, " ", birthName.LastName);
                            }
                        }
            ...
            if (personalNames.Any())
            {
                ...
                if (chosenNames.Any())
                {
                    ...
                    if (chosenName != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(chosenName.LastNamePrefix) && !string.IsNullOrEmpty(chosenName.LastName))
                        {
                            ...
                            {
                                if (!string.IsNullOrEmpty(chosenName.LastNamePrefix) && !chosenName.LastNamePrefix.Equals(chosenName.LastName, StringComparison.OrdinalIgnoreCase))
                                {
                                    chosenName.LastName = string.Concat(chosenName.LastNamePrefix, " ", chosenName.LastName);
                                }
                            }
                ...
                foreach (var historyName in historyNames)
                {
                    ...
                    if (!string.IsNullOrEmpty(historyName.LastNamePrefix) && !string.IsNullOrEmpty(historyName.LastName))
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(historyName.LastNamePrefix) && !historyName.LastNamePrefix.Equals(historyName.LastName, StringComparison.OrdinalIgnoreCase))
                            {
                                historyName.LastName = string.Concat(historyName.LastNamePrefix, " ", historyName.LastName);
                            }
                        }
                ...
                else
                {
                    ...
                    foreach (var historyName in historyNames)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            logger.Error(ex.Message, "Missing last name in constructor or object reference error.");
                        }
                    }
                    if (personEntity != null)
                    {
        ...
        /// <param name="personMaritalStatusDtoProperty"></param>
        /// <returns>Domain.Base.Entities.PersonIntegration</returns>
        private async Task<Domain.Base.Entities.PersonIntegration> ConvertPerson2MaritalStatus2DtoToEntityAsync(PersonIntegration personEntity, Dtos.DtoProperties.PersonMaritalStatusDtoProperty personMaritalStatusDtoProperty)
        {
            try
            {
                var isDefined = Enum.IsDefined(typeof(Dtos.EnumProperties.PersonMaritalStatusCategory), personMaritalStatusDtoProperty.MaritalCategory);
            ...
            if (personMaritalStatusDtoProperty.Detail != null && !string.IsNullOrEmpty(personMaritalStatusDtoProperty.Detail.Id))
            {
                ...
                try
                {
                    ...
                    {
                        //
                        // Allow for the possibility of a dto marital status GUID with a null type when dto marital category is "Other"
                        //  because Ethos integration treats a marital statuses with no special processing or special processing "7" as "other".
                        //
                        if (!(maritalStatusEntityById.Type == null && maritalStatusCategory == Domain.Base.Entities.MaritalStatusType.Other))
                        {
                            throw new InvalidOperationException("maritalStatus.maritalCategory '" + maritalStatusCategory + "' does not match the category '" + maritalStatusEntityById.Type + "' of id: " + personMaritalStatusDtoProperty.Detail.Id);
                        }
                    }
                    maritalState = (Domain.Base.Entities.MaritalState?)maritalStatusEntityById.Type;
                    personEntity.MaritalStatus = maritalState;
            ...
            {
                // This logic is encountered only when person does not already have a marital status (and we have an incoming category with no detail).  Otherwise partial put
                // would have added personMaritalStatusDtoProperty.Detail from the person's existing marital status.
                //
                // Translate the marital status enumeration for the DTO to the marital status enumeration for the entity.

                try
                {
                    maritalState = (Domain.Base.Entities.MaritalState)Enum.Parse(typeof(Domain.Base.Entities.MaritalState), personMaritalStatusDtoProperty.MaritalCategory.ToString());
                ...
                if (reporting.Country != null && reporting.Country.EthnicCategory != null)
                {
                    try
                    {
                        ...
                        if (reporting.Country != null && reporting.Country.EthnicCategory != null && reporting.Country.EthnicCategory.HasValue)
                        {
                            ...
                            {
                                //
                                // Only issue error if we have a type.  If not, means it has special processing of NRA
                                // (from ConvertEthnicityTypeCodeToEthnicityType of ReferenceDataRepository)
                                // and NRA ethnicities should suppress the reporting object.
                                //
                                if (ethnicityEntity.Type != null)
                                {
        ...
        private void ValidateAddressDataAsync(IEnumerable<PersonAddressDtoProperty> addressList, IEnumerable<Domain.Base.Entities.County> countiesCodes)
        {
                ...
                foreach (var addressDto in addressList)
                {
                    ...
                    if (string.IsNullOrEmpty(addressDtoAddress.Id) || addressDtoAddress.Id == Guid.Empty.ToString())
                    {
                        if (addressDtoAddress.AddressLines == null || !addressDtoAddress.AddressLines.Any())
                            throw new ArgumentNullException("addressDto.addressLines", "Street address lines are required.");

                        if (addressDtoAddress.Place != null)
                        {
                            if (addressDtoAddress.Place.Country == null || addressDtoAddress.Place.Country.Code == null)
                            {
                            ...
                            var addressCountry = addressDtoAddress.Place.Country;
                            // Require sub-region code if we have a sub-region title
                            if (addressCountry.SubRegion != null && !string.IsNullOrEmpty(addressCountry.SubRegion.Title) && string.IsNullOrEmpty(addressCountry.SubRegion.Code))
                            {
                                if (!string.IsNullOrEmpty(addressCountry.SubRegion.Title) && (addressCountry.Code == IsoCode.USA || addressCountry.Code == IsoCode.CAN))
                                {
                                    var counties = (countiesCodes).Where(c => c.Description.Equals(addressCountry.SubRegion.Title, StringComparison.OrdinalIgnoreCase));
                                    if (counties == null || counties.Count() != 1)
                                    {
                                        throw new ArgumentNullException("addressDto.place.country.subRegion.code", "address subregion code is required.");
                                    }
                                }
                            }
                        }
            ...
            if (addressList != null && addressList.Any())
            {
                ...
                foreach (var addressDto in addressList)
                {
                    ...
                    if (addressDto.address.Id != null && addressDto.address.Id != Guid.Empty.ToString())
                    {
                        ...
                        try
                        {
                            addressId = await _personRepository.GetAddressIdFromGuidAsync(addressDto.address.Id);
                        }
                        catch (Exception ex)
                        {
                            //ignore all errors as it will be caught below
                    ...
                    if (string.IsNullOrEmpty(addressDtoAddress.Id) || addressDtoAddress.Id == Guid.Empty.ToString())
                    {
                        ...
                        if (addressDtoAddress.Place != null)
                        {
                            ...
                            if (addressCountry.SubRegion != null)
                            {
                                if (!string.IsNullOrEmpty(addressCountry.SubRegion.Code))
                                {
                                ...
                                else
                                {
                                    if (!string.IsNullOrEmpty(addressCountry.SubRegion.Title) && (addressCountry.Code == IsoCode.USA || addressCountry.Code == IsoCode.CAN))
                                    {
                                        var counties = (await GetCountiesAsync(false)).Where(c => c.Description.Equals(addressCountry.SubRegion.Title, StringComparison.OrdinalIgnoreCase));
                                        if (counties != null && counties.Count() == 1)
                                        {
        ...
        private async Task<IEnumerable<Domain.Base.Entities.Phone>> ConvertPhoneDtoToPhoneEntities2(IEnumerable<PersonPhone2DtoProperty> phoneList)
        {
            var phoneEntities = new List<Domain.Base.Entities.Phone>();
            if (phoneList != null && phoneList.Any())
            {
        ...
        private async Task<Domain.Base.Entities.PersonIntegration> ConvertOrganization2DtoToPersonEntityAsync(string personOrgId, Dtos.Organization2 organizationDto)
        {
            ...
            if (string.IsNullOrEmpty(organizationDto.Title))
            ...
            Domain.Base.Entities.PersonIntegration personEntity, bool bypassCache = false)
        {
            ...
            if (string.IsNullOrEmpty(personEntity.Guid))
                IntegrationApiExceptionAddError(string.Format("Unable to find the GUID for the organization '{0}'", personEntity.Id), "GUID.Not.Found", personEntity.Guid, personEntity.Id);
            else
                organizationDto.Id = personEntity.Guid;
            if (string.IsNullOrEmpty(personEntity.PreferredName))
                IntegrationApiExceptionAddError(string.Format("Title is required'", personEntity.Id), "Bad.Data", personEntity.Guid, personEntity.Id);
            else
            ...
            catch (Exception ex)
            {
                IntegrationApiExceptionAddError(ex.Message, "GUID.Not.Found",
                   personEntity.Guid, personEntity.Id);
            }
            if (addressEntities != null)
            {
                try
                {
                    var addresses = await GetOrganizationAddressesAsync(addressEntities.Where(a => a.AddressLines != null && a.AddressLines.Any()).ToList(), personEntity.Guid, personEntity.Id, bypassCache);
            ...
            try
            {
                ...
                {
                    organizationDto.Phones = phoneNumbers;
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="personId"></param>
        /// <returns></returns>
        public async Task<GuidObject2> GetPersonGuidByIdAsync(string personId)
        {
            if (!CurrentUser.IsPerson(personId))
            {
                throw new PermissionsException("Person GUID retrieval not permitted by other users.");
            }

            var personGuid = await _personRepository.GetPersonGuidFromIdAsync(personId);
            if (string.IsNullOrEmpty(personGuid))
            {
                throw new KeyNotFoundException("Could not find GUID for that person ID.");
            }

            var guidObject = new GuidObject2(personGuid);
            return guidObject;
        }
    }
}
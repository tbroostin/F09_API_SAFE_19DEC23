...
{
    public class ProgramEvaluationDtoAdapter : BaseAdapter<Domain.Student.Entities.ProgramEvaluation, Dtos.Planning.ProgramEvaluation>
    {
        /// <summary>
        /// This class maps a program evaluation entity to an outbound program evaluation DTO
        /// </summary>
        ///
        public ProgramEvaluationDtoAdapter(IAdapterRegistry adapterRegistry, ILogger logger) : base(adapterRegistry, logger) { }

        public override Dtos.Planning.ProgramEvaluation MapToType(Domain.Student.Entities.ProgramEvaluation Source)
        {

            var programEvaluationDto = new Dtos.Planning.ProgramEvaluation();
            ...
            programEvaluationDto.InstGpa = Source.InstGpa ?? 0;
            programEvaluationDto.CumGpa = Source.CumGpa ?? 0;
            ...
            programEvaluationDto.RequirementResults = new List<RequirementResult>();
                
            // map requirement results
            if (Source.RequirementResults.Count > 0)
            {
                RequirementResultDtoAdapter requirementResultDtoAdapter = new RequirementResultDtoAdapter(adapterRegistry, logger);
                programEvaluationDto.RequirementResults.AddRange(Source.RequirementResults.Select(rr => requirementResultDtoAdapter.MapToType(rr)));
            }

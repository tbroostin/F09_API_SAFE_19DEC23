...
{
    [TestClass]
    public class AcademicHistoryEntityAdapterTests
    {
        Ellucian.Colleague.Domain.Student.Entities.AcademicHistory academicHistoryEntity;
        AcademicHistoryEntityAdapter adapter;
        ...
        public async void Initialize()
        {
            ...
            adapter = new AcademicHistoryEntityAdapter(adapterRegistry, loggerMock.Object);
        }

        [TestMethod]
        public void AcademicHistoryEntityAdapter_SuccessfullyConvertsToDto()
        {
            AcademicHistory academicHistoryDto = adapter.MapToType(academicHistoryEntity);
            Assert.IsTrue(academicHistoryDto.GradeRestriction.IsRestricted);
            Assert.IsTrue(academicHistoryDto.AcademicTerms.Count() > 0);
            Assert.IsTrue(academicHistoryDto.NonTermAcademicCredits != null);
        }
    }
}

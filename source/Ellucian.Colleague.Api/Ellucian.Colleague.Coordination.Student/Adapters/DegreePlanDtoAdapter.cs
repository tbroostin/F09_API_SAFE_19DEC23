...
namespace Ellucian.Colleague.Coordination.Student.Adapters
{
    ...
    /// </summary>
    public class DegreePlanDtoAdapter : BaseAdapter<Dtos.Student.DegreePlans.DegreePlan, Domain.Student.Entities.DegreePlans.DegreePlan>
    {
        ...
        public override Domain.Student.Entities.DegreePlans.DegreePlan MapToType(Dtos.Student.DegreePlans.DegreePlan Source)
        {
                        ...
                        if (tc.PlannedCourses != null && tc.PlannedCourses.Count() > 0)
                        {
                            foreach (var plannedcourse in tc.PlannedCourses)
                            {
                                if (!string.IsNullOrEmpty(plannedcourse.CourseId))
                                {
                                    ...
                                    degreePlanEntity.AddCourse(new Domain.Student.Entities.DegreePlans.PlannedCourse(plannedcourse.CourseId, plannedcourse.SectionId, gradingType, waitStatus) { Credits = plannedcourse.Credits }, tc.TermId);
                                }
                            }
                        }
                    }
            ...
            if (Source.NonTermPlannedCourses != null)
            {
                foreach (var ntpc in Source.NonTermPlannedCourses)
                {
                    if (!string.IsNullOrEmpty(ntpc.CourseId))
                    {
                        ...
                        degreePlanEntity.AddCourse(new Domain.Student.Entities.DegreePlans.PlannedCourse(ntpc.CourseId, ntpc.SectionId, gradingType, waitStatus) { Credits = ntpc.Credits }, null);
                    }
                }
            }

...
{
    public class RequirementResultDtoAdapter : BaseAdapter<Ellucian.Colleague.Domain.Student.Entities.Requirements.RequirementResult, Ellucian.Colleague.Dtos.Student.Requirements.RequirementResult>
    {
        public RequirementResultDtoAdapter(IAdapterRegistry adapterRegistry, ILogger logger) : base(adapterRegistry, logger) { }

        public override Ellucian.Colleague.Dtos.Student.Requirements.RequirementResult MapToType(Ellucian.Colleague.Domain.Student.Entities.Requirements.RequirementResult Source)
        {
            Ellucian.Colleague.Dtos.Student.Requirements.RequirementResult requirementResult = new Ellucian.Colleague.Dtos.Student.Requirements.RequirementResult();

            requirementResult.RequirementId = Source.Requirement.Id;
            requirementResult.Gpa = Source.Gpa ?? 0;
            ...
            requirementResult.SubrequirementResults = new List<Ellucian.Colleague.Dtos.Student.Requirements.SubrequirementResult>();
            var SubrequirementResultDtoAdapter = new SubrequirementResultDtoAdapter(adapterRegistry, logger);

            if (Source.SubRequirementResults.Count > 0)
            {
                requirementResult.SubrequirementResults.AddRange(Source.SubRequirementResults.Select(sr => SubrequirementResultDtoAdapter.MapToType(sr)).ToList());
            }

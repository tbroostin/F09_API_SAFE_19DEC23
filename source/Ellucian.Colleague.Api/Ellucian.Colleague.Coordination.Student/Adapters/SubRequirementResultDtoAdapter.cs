...
{
    public class SubrequirementResultDtoAdapter : BaseAdapter<Ellucian.Colleague.Domain.Student.Entities.Requirements.SubrequirementResult, Ellucian.Colleague.Dtos.Student.Requirements.SubrequirementResult>
    {
        public SubrequirementResultDtoAdapter(IAdapterRegistry adapterRegistry, ILogger logger) : base(adapterRegistry, logger) { }

        public override Ellucian.Colleague.Dtos.Student.Requirements.SubrequirementResult MapToType(Ellucian.Colleague.Domain.Student.Entities.Requirements.SubrequirementResult Source)
        {
            var subrequirementResult = new Ellucian.Colleague.Dtos.Student.Requirements.SubrequirementResult();

            subrequirementResult.SubrequirementId = Source.SubRequirement.Id;
            subrequirementResult.Gpa = Source.Gpa ?? 0;
            ...
            subrequirementResult.GroupResults = new List<Ellucian.Colleague.Dtos.Student.Requirements.GroupResult>();

            var groupResultDtoAdapter = new GroupResultDtoAdapter(adapterRegistry, logger);

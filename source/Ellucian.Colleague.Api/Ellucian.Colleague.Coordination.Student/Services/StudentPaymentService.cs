...
namespace Ellucian.Colleague.Coordination.Student.Services
{
    ...
    public class StudentPaymentService : BaseCoordinationService, IStudentPaymentService
    {
        ...
        /// <param name="id">Guid to General Ledger Transaction</param>
        /// <returns>General Ledger Transaction DTO</returns>
        public async Task<Dtos.StudentPayment3> GetByIdAsync3(string id)
        {
            CheckViewStudentPaymentsPermission();
            // Get the student payments domain entity from the repository
            Ellucian.Colleague.Domain.Student.Entities.StudentPayment studentPaymentDomainEntity = null;
            ...
            // Convert the student payment object into DTO.
            StudentPayment3 studentPaymentDto = null;
            try
            {
                studentPaymentDto = await BuildStudentPaymentDtoAsync3(studentPaymentDomainEntity, personGuidCollection, true);
            }
            catch (Exception ex)
            {
                IntegrationApiExceptionAddError(ex.Message, "Global.Internal.Error", studentPaymentDomainEntity.Guid, studentPaymentDomainEntity.RecordKey);
        ...
        /// <param name="usage">Records from AR.PAY.ITEMS.INTG where Usage matches "taxReportingOnly".</param>
        /// <returns>Collection of StudentPayments</returns>
        public async Task<Tuple<IEnumerable<Dtos.StudentPayment3>, int>> GetAsync3(int offset, int limit, bool bypassCache, string student = "", string academicPeriod = "", string fundSource = "", string paymentType = "", string fundDestination = "", string usage = "")
        {
            CheckViewStudentPaymentsPermission();

            var studentPaymentDtos = new List<Dtos.StudentPayment3>();
            var personId = string.Empty;
            var term = string.Empty;
            var distrCode = string.Empty;
            var arType = string.Empty;
            ...
            if (!string.IsNullOrEmpty(student))
            {
                ...
                {
                    return new Tuple<IEnumerable<Dtos.StudentPayment3>, int>(studentPaymentDtos, 0);
                }
                if (string.IsNullOrEmpty(personId))
                {
                    return new Tuple<IEnumerable<Dtos.StudentPayment3>, int>(studentPaymentDtos, 0);
            ...
            foreach (var studentPaymentDomainEntity in studentPaymentDomainEntities)
            {
                try
                {
                    studentPaymentDtos.Add(await BuildStudentPaymentDtoAsync3(studentPaymentDomainEntity, personGuidCollection, bypassCache));
                }
                catch (Exception ex)
                {
                    IntegrationApiExceptionAddError(ex.Message, "Global.Internal.Error", studentPaymentDomainEntity.Guid, studentPaymentDomainEntity.RecordKey);
            ...
            return new Tuple<IEnumerable<Dtos.StudentPayment3>, int>(studentPaymentDtos, studentPaymentDomainTuple.Item2);
        }
        ...
        /// </summary>
        /// <returns>A single StudentPayment</returns>
        public async Task<Dtos.StudentPayment3> CreateAsync3(Dtos.StudentPayment3 studentPayment)
        {
            CheckCreateStudentPaymentsPermission();

            Dtos.StudentPayment3 studentPaymentDto = null;
            studentPaymentRepository.EthosExtendedDataDictionary = EthosExtendedDataDictionary;
            ...
            try
            {
                studentPaymentEntityRequest = await BuildStudentPaymentEntityAsync3(studentPayment);
            }
            catch (Exception ex)
            {
                IntegrationApiExceptionAddError("Record not created.  Error extracting request. " + ex.Message, "Global.Internal.Error",
            ...
            try
            {
                studentPaymentDto = await BuildStudentPaymentDtoAsync3(studentPaymentEntityResponse, personGuidCollection, true);
            }
            catch (Exception ex)
            {
                IntegrationApiExceptionAddError("Record created. Error building response. " + ex.Message, "Global.Internal.Error", studentPaymentEntityResponse.Guid, studentPaymentEntityResponse.RecordKey);
        ...
        /// <summary>
        /// BuildStudentPaymentDtoAsync3
        /// </summary>
        /// <param name="studentPaymentEntity"></param>
        /// <param name="personGuidCollection"></param>
        /// <param name="bypassCache"></param>
        /// <returns></returns>
        private async Task<Dtos.StudentPayment3> BuildStudentPaymentDtoAsync3(Ellucian.Colleague.Domain.Student.Entities.StudentPayment studentPaymentEntity,
            Dictionary<string, string> personGuidCollection, bool bypassCache = true)
        {
            ...
            var studentPaymentDto = new Dtos.StudentPayment3();
            studentPaymentDto.Id = string.IsNullOrEmpty(studentPaymentEntity.Guid) ? Guid.Empty.ToString() : studentPaymentEntity.Guid;
            ...
            studentPaymentDto.PaidOn = studentPaymentEntity.PaymentDate;
            studentPaymentDto.PaymentType = !string.IsNullOrEmpty(studentPaymentEntity.PaymentType) ?
                ConvertPaymentTypes2(studentPaymentEntity.PaymentType) :
                Dtos.EnumProperties.StudentPaymentTypes2.notset;
            studentPaymentDto.Comments = studentPaymentEntity.Comments != null && studentPaymentEntity.Comments.Any() ?
                studentPaymentEntity.Comments :
                null;
        ...
        /// <param name="bypassCache"></param>
        /// <returns></returns>
        private async Task<Ellucian.Colleague.Domain.Student.Entities.StudentPayment> BuildStudentPaymentEntityAsync3(Dtos.StudentPayment3 studentPaymentDto, bool bypassCache = true)
        {
            if (studentPaymentDto == null)
            {
                IntegrationApiExceptionAddError("StudentPayment body is required.", "Missing.Request.Body");
            ...
            if (studentPaymentDto.PaymentType == Dtos.EnumProperties.StudentPaymentTypes2.notset)
            {
                //throw new ArgumentNullException("studentPayment.paymentType", "The paymentType must be set and cannot be null. ");
                IntegrationApiExceptionAddError("studentPayment.paymentType is required.", "Missing.Required.Property", studentPaymentDto.Id);
            }
        ...
        /// <returns></returns>
        private Dtos.EnumProperties.StudentPaymentTypes2 ConvertPaymentTypes2(string paymentType)
        {
            switch (paymentType.ToLowerInvariant())
            {
                case "deposit":
                    {
                        return Dtos.EnumProperties.StudentPaymentTypes2.deposit;
                    }
                case "sponsor":
                    {
                        return Dtos.EnumProperties.StudentPaymentTypes2.sponsor;
                    }
                case "payroll":
                    {
                        return Dtos.EnumProperties.StudentPaymentTypes2.payroll;
                    }
                case "cash":
                    {
                        return Dtos.EnumProperties.StudentPaymentTypes2.cash;
                    }
                default:
                    {
                        return Dtos.EnumProperties.StudentPaymentTypes2.notset;
                    }
            }
        }

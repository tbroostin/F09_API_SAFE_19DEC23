...
namespace Ellucian.Colleague.Data.Base.Tests.Repositories
{
    ...
    {
        [TestClass]
        public class AddressRepositoryTests_GetAddresses2Async
        {
            ...
            protected Collection<Address> cleanAddressResponseData;
            ...
            [TestMethod]
            public async Task GetAddressFromGuid2Async_Success()
            {
            ...
            public async Task AddressRepository_Get_RepositoryException2()
            {

                dataAccessorMock.Setup(d => d.BulkReadRecordWithInvalidKeysAndRecordsAsync<DataContracts.Address>(It.IsAny<string>(), It.IsAny<string[]>(), It.IsAny<bool>()))
                   .ThrowsAsync(new RepositoryException());

                var actuals = await addressRepo.GetAddresses2Async(0, 200, new string[] { "1" });
            }

            [TestMethod]
            [ExpectedException(typeof(RepositoryException))]
            public async Task AddressRepository_Get_WithBadRecords_RepositoryException2()
            {
                var results = new Ellucian.Data.Colleague.BulkReadOutput<DataContracts.Address>()
                { BulkRecordsRead = cleanAddressResponseData };
                results.InvalidKeys = new[] { "102" };
                Dictionary<string, string> rec = new Dictionary<string, string>();
                rec.Add("102", "102");
                results.InvalidRecords = rec;
                dataAccessorMock.Setup(d => d.BulkReadRecordWithInvalidKeysAndRecordsAsync<DataContracts.Address>(It.IsAny<string>(), It.IsAny<string[]>(), It.IsAny<bool>()))
                    .ReturnsAsync(results);


                var actuals = await addressRepo.GetAddresses2Async(0, 200, null);
            }

            [TestMethod]
            //[ExpectedException(typeof(RepositoryException))]
            public async Task AddressRepository_Get_NoAddrLine2_NoException()
            {
                var results = new Ellucian.Data.Colleague.BulkReadOutput<DataContracts.Address>()
                { BulkRecordsRead = cleanAddressResponseData };
                results.BulkRecordsRead.FirstOrDefault().AddressLines = new List<string>() { };
                dataAccessorMock.Setup(d => d.BulkReadRecordWithInvalidKeysAndRecordsAsync<DataContracts.Address>(It.IsAny<string>(), It.IsAny<string[]>(), It.IsAny<bool>()))
                    .ReturnsAsync(results);
            ...
            [TestMethod]
            public async Task AddressRepository_GetAddresses2()
            {
                var addresses = await addressRepo.GetAddresses2Async(0, 100, null);
                Assert.IsNotNull(addresses);

                var expected = cleanAddressResponseData;
                Assert.AreEqual(expected.Count, addresses.Item2);
                Assert.AreEqual(expected.FirstOrDefault().RecordGuid, addresses.Item1.FirstOrDefault().Guid);
            }

            [TestMethod]
            public async Task AddressRepository_WithPersonFilterIds_GetAddresses2()
            {
                var personFiltIds = new List<string>() { "0003397", "0000010" };
                var addresses = await addressRepo.GetAddresses2Async(0, 100, personFiltIds.ToArray());
                Assert.IsNotNull(addresses);
            ...
            private AddressRepository BuildValidRepository()
            {
                ...
                dataAccessorMock.Setup<Task<Address>>(acc => acc.ReadRecordAsync<Address>("ADDRESS", addressId, true)).ReturnsAsync(addressResponse.ElementAt(0));
                ...
                //  Setup for Get All
                dataAccessorMock.Setup(a => a.SelectAsync("ADDRESS", It.IsAny<string[]>(), It.IsAny<string>())).ReturnsAsync(addressIds.ToArray());
                var results = new Ellucian.Data.Colleague.BulkReadOutput<DataContracts.Address>() { BulkRecordsRead = cleanAddressResponseData };
                dataAccessorMock.Setup(d => d.BulkReadRecordWithInvalidKeysAndRecordsAsync<DataContracts.Address>(It.IsAny<string>(), It.IsAny<string[]>(), It.IsAny<bool>())).ReturnsAsync(results);
                ...
                GetCacheApiKeysResponse resp = new GetCacheApiKeysResponse()
                {
                    Offset = 0,
                    Limit = 2,
                    CacheName = "AllAddresses:",
                    Entity = "Address",
                    Sublist = new List<string>() { "101", "102", "103", "104", "105", "106", "107" },
                    TotalCount = 7,
                ...
                dataAccessorMock.Setup(a => a.SelectAsync("PERSON", It.IsAny<string[]>(), It.IsAny<string>())).ReturnsAsync(addressIds.ToArray());
        ...
        [TestClass]
        public class AddressRepositoryTests_Get
        {
            protected List<string> personIds;
            protected List<string> addressIds;
            protected Dictionary<string, Person> personRecords;
        ...
        [TestClass]
        public class AddressRepositoryTests_Update
        {
            protected List<string> personIds;
            protected List<string> addressIds;
            protected Dictionary<string, Person> personRecords;
            ...
            public async Task AddressRepository_Update()
            {
                ...
                Domain.Base.Entities.Address addressEntity = new Domain.Base.Entities.Address
                    (id,true)
                {
                    AddressChapter = new List<string>() { "1", "2" },
                    AddressLabel = new List<string>() { "1", "2" },
                ...
                Address addressDataContract = new Address()
                {
                    RecordGuid = guid,
                    Recordkey = id,
                    AddressLines = new List<string>() { "123 main st" }
                ...
                dataAccessorMock.Setup(acc => acc.ReadRecordAsync<Address>(id, true)).ReturnsAsync(addressDataContract);

                UpdateAddressResponse updateResponse = new UpdateAddressResponse()
                { AddressId = addressEntity.AddressId,
                    Guid = addressEntity.Guid};

                iColleagueTransactionInvokerMock.Setup(i => i.ExecuteAsync<UpdateAddressRequest, UpdateAddressResponse>
                (It.IsAny<UpdateAddressRequest>())).ReturnsAsync(updateResponse);

                var result = await addressRepo.UpdateAsync(addressEntity.AddressId, addressEntity);
                Assert.IsNotNull(result);
                Assert.IsNotNull(result.Guid, guid, "Guid");
            }

            [TestMethod]
            [ExpectedException(typeof(ArgumentNullException))]
            public async Task AddressRepository_Update_MissingAddressLines()
            {
                Collection<DataContracts.Address> Addressdb = new Collection<DataContracts.Address>();
                string id, guid;
                GuidLookup guidLookup;
            ...
            [TestMethod]
            public async Task AddressRepository_Update2_NoAddressLines()
            {
                Collection<DataContracts.Address> Addressdb = new Collection<DataContracts.Address>();
                string id, guid;
                GuidLookup guidLookup;
                ...
                var result = await addressRepo.Update2Async(addressEntity.AddressId, addressEntity);
                Assert.IsNotNull(result);
                Assert.IsNotNull(result.Guid, guid, "Guid");
            }
        ...
        [TestClass]
        public class AddressRepositoryTests_GetPersonAddressesAsync
        {
            protected List<string> personIds;
            protected List<string> addressIds;
            protected Dictionary<string, Person> personRecords;
            ...
            public async Task AddressReposoitory_Get_ArgumentOutOfRangeException()
            {
                var actuals = await addressRepo.GetPersonAddressesAsync("personId");
            }
            ...
            public async Task AddressReposoitory_Get_RepositoryException()
            {
                ...
                var actuals = await addressRepo.GetPersonAddressesAsync("0000304");
            }

        
            [TestMethod]
            public async Task GetPersonAddressesAsync_CheckSingleAddressProperties_Valid()
            {
                IEnumerable<Ellucian.Colleague.Domain.Base.Entities.Address> addresses = await addressRepo.GetPersonAddressesAsync(personIds.ElementAt(0));
                Ellucian.Colleague.Domain.Base.Entities.Address address = addresses.ElementAt(0);
                Assert.AreEqual("101", address.AddressId);
                Assert.AreEqual("65498 Ft. Belvoir Hwy", address.AddressLines.ElementAt(0));
                Assert.AreEqual("Mount Vernon", address.AddressLines.ElementAt(1));
            ...
            [TestMethod]
            public async Task SinglePersonAddressCount_Valid()
            {
                var personId = personIds.ElementAt(0);
                IEnumerable<Ellucian.Colleague.Domain.Base.Entities.Address> addresses = await addressRepo.GetPersonAddressesAsync(personId);
                Assert.AreEqual(3, addresses.Count());
            }
            ...
            public async Task GetAddressbyIDAsync_ArgumentNullException()
            {
                ...
                await addressRepo.GetPersonAddressesAsync(null);
            }
            ...
            private AddressRepository BuildValidRepository()
            {
                ...
                // Mock the call for getting multiple person records
                personResponseData = BuildPersonResponseData(personRecords);
                dataAccessorMock.Setup(acc => acc.BulkReadRecordAsync<Person>("PERSON", It.IsAny<string[]>(), true)).ReturnsAsync(personResponseData);
                ...
                dataAccessorMock.Setup(acc => acc.BulkReadRecordAsync<Address>("ADDRESS", addressIds.ToArray(), true)).ReturnsAsync(addressResponse);
                dataAccessorMock.Setup(acc => acc.BulkReadRecordAsync<Address>("ADDRESS", allPersonAddressIds, true)).ReturnsAsync(addressResponse);
                dataAccessorMock.Setup(acc => acc.BulkReadRecordAsync<Address>("ADDRESS", personAddressIds, true)).ReturnsAsync(person1AddressResponse);

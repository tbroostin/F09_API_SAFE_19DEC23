...
namespace Ellucian.Colleague.Data.ColleagueFinance.Repositories
{
    ...
    public class ColleagueFinanceTaxFormStatementRepository : BaseColleagueRepository, IColleagueFinanceTaxFormStatementRepository
    {
        ...
        public async Task<IEnumerable<TaxFormStatement3>> Get2Async(string personId, string taxForm)
        {
            ...
            switch (taxForm)
            {
                case TaxFormTypes.FormT4A:
                    statements = (await GetT4ATaxStatements2Async(personId)).ToList();
                    break;

                case TaxFormTypes.Form1099MI:
                    statements = (await Get1099MiTaxStatements2Async(personId)).ToList();
                    break;
                case TaxFormTypes.Form1099NEC:
                    statements = (await Get1099NecTaxStatementsAsync(personId)).ToList();
                    break;
                default:
        ...
        private async Task<IEnumerable<TaxFormStatement3>> GetT4ATaxStatements2Async(string personId)
        {
            var statements = new List<TaxFormStatement3>();
                        ...
                        if (taxRecord != null && !string.IsNullOrEmpty(taxRecord.TftbrBinId) && taxRecord.TftbrBinId == personId && taxRecord.TftbrYear.HasValue)
                        {
                            ...
                            if (detailTaxRecords != null && detailTaxRecords.Any())
                            {
                                ...
                                if (isIncluded)
                                {
                                    ...
                                    {
                                        var statementT4a = new TaxFormStatement3(taxRecord.TftbrBinId,
                                            taxRecord.TftbrYear.ToString(), TaxFormTypes.FormT4A, taxRecord.Recordkey);
        ...
        private async Task<IEnumerable<TaxFormStatement3>> Get1099MiTaxStatements2Async(string personId)
        {
                ...
                foreach (var taxFormData in list)
                {
                    ...
                    if (miYear != null)
                    {
                        ...
                        if (miLatestRecord != null)
                        {
                            ...
                            AddRecordToStatements2(statements, miLatestRecord, miYear);
                        }
                    }
                }
            }
        ...
        private static void AddRecordToStatements2(List<TaxFormStatement3> statements, Tax1099miDetailRepos miLatestRecord, TaxForm1099miYears miYear)
        {
            var latestStatement = new TaxFormStatement3(miLatestRecord.TmidtlrVendorId,
               miLatestRecord.TmidtlrYear.ToString(), TaxFormTypes.Form1099MI, miLatestRecord.Recordkey, miLatestRecord.TmidtlrStateId);
        ...
        /// <summary>
        /// Obtain the available list of 1099-NEC statements for this person.
        /// </summary>
        /// <param name="personId"></param>
        /// <returns>List of 1099NEC statement objects.</returns>
        private async Task<IEnumerable<TaxFormStatement3>> Get1099NecTaxStatementsAsync(string personId)
        {
            // Initialize the statements to return.
            var statements = new List<TaxFormStatement3>();
            ...
            // Read the tax year records for 1099NEC.
            Collection<TaxForm1099necYears> taxYearRecords = await DataReader.BulkReadRecordAsync<TaxForm1099necYears>("TAX.FORM.1099NEC.YEARS", "");

            // Find the all the 1099NEC tax forms detail records for the recipient.
            var taxFormDetailCriteria = "WITH TNEDTLR.VENDOR.ID EQ '" + personId + "'";
            var taxFormDetailRepos = await DataReader.BulkReadRecordAsync<Tax1099necDetailRepos>(taxFormDetailCriteria);
            ...
            // A temporary filter the recipient detail records to get only those for which we have tax years template is ready in the product
            // This will be removed once all the tax year templates are ready for 1099NEC tax forms
            string[] includedYearsFor1099Nec = ColleagueFinanceConstants.IncludedYearsFor1099nec.Split(',').Select(s => s.Trim()).ToArray();
            var filteredTaxFormDetailRepos = taxFormDetailRepos.Where(taxform => taxform != null && includedYearsFor1099Nec.Contains(taxform.TnedtlrYear));
            Tax1099necDetailRepos necLatestRecord = null;
            try
            {
                // Get a distinct list of tax forms by Year-State-Ein by descending tax year.
                var list = filteredTaxFormDetailRepos.Select(t => new { t.TnedtlrYear, t.TnedtlrStateId, t.TnedtlrEin }).Distinct().OrderByDescending(x => x.TnedtlrYear).ThenBy(x => x.TnedtlrStateId);

                foreach (var taxFormData in list)
                {
                    // Get the TAX.FORM.1099NEC.YEARS record for the tax year in this tax form.
                    var necYear = taxYearRecords.FirstOrDefault(x => x.Recordkey == taxFormData.TnedtlrYear);
                    // We need the tax year record to see if the web enabled flag is set to Yes for the notation.
                    if (necYear != null)
                    {
                        // Get the recipient's detail records for this tax year, state and EIN.
                        var necRecords = filteredTaxFormDetailRepos.Where(x => x.TnedtlrYear == taxFormData.TnedtlrYear && x.TnedtlrStateId == taxFormData.TnedtlrStateId && x.TnedtlrEin == taxFormData.TnedtlrEin && x.TnedtlrQualifiedFlag == "Y");
                        if (necRecords != null && necRecords.Any())
                        {
                            // Check if the record is for the current tax year.
                            if (parm1099NECContract.P1099necYear == taxFormData.TnedtlrYear)
                            {
                                if (necRecords.Any(x => x.TnedtlrRefId == "M" && x.TnedtlrQualifiedFlag == "Y" && x.TnedtlrStatus != "U"))
                                {
                                    necLatestRecord = necRecords.Where(x => x.TnedtlrRefId == "M").FirstOrDefault();
                                }
                                else
                                {
                                    // The tax form year is current year which do not have valid M Record
                                    // Get the record that has the most recent submit sequence number with valid status 
                                    necLatestRecord = GetMostRecentSubmittedValidRecordFor1099Nec(necLatestRecord, necYear, necRecords);
                                }
                            }
                            else
                            {
                        ...
                        if (necLatestRecord != null)
                        {
                            if (necLatestRecord.Recordkey == null)
                            {
                                throw new NullReferenceException("RecordKey is a required field");
                            }
                            //Add the record to the statement
                            AddRecordToStatementsFor1099Nec(statements, necLatestRecord, necYear);
                        }
                    }
                }
            }
        ...
        private static Tax1099necDetailRepos GetMostRecentSubmittedValidRecordFor1099Nec(Tax1099necDetailRepos necLatestRecord, TaxForm1099necYears necYear, IEnumerable<Tax1099necDetailRepos> necRecords)
        {
            if (necYear.TfneySubmitSeqNos.Any())
            {
                foreach (var seqNo in necYear.TfneySubmitSeqNos)
                {
                    necLatestRecord = necRecords.Where(x => x.TnedtlrRefId == seqNo && x.TnedtlrStatus != "U").FirstOrDefault();
                    if (necLatestRecord != null)
                        break;
                }
            }
            return necLatestRecord;
        }

        private static void AddRecordToStatementsFor1099Nec(List<TaxFormStatement3> statements, Tax1099necDetailRepos necLatestRecord, TaxForm1099necYears necYear)
        {
            var latestStatement = new TaxFormStatement3(necLatestRecord.TnedtlrVendorId,
               necLatestRecord.TnedtlrYear.ToString(), TaxFormTypes.Form1099NEC, necLatestRecord.Recordkey, necLatestRecord.TnedtlrStateId);
            //Set the notation for the statement
            latestStatement.Notation = necYear.TfneyWebEnabled == "Y" && necLatestRecord.TnedtlrQualifiedFlag == "Y" ? TaxFormNotations.None : TaxFormNotations.NotAvailable;
            statements.Add(latestStatement);
        }
        ...
        [Obsolete("Obsolete as of API 1.29.1. Use GetT4ATaxStatements2Async instead.")]
        ...
        /// <param name="personId"></param>
        /// <returns>List of 1099MI statement objects.</returns>
        [Obsolete("Obsolete as of API 1.29.1. Use Get1099MiTaxStatements2Async instead.")]
        ...
        [Obsolete("Obsolete as of API 1.29.1. Use AddRecordToStatements2 instead.")]

                        ...
                        foreach (var contract in filteredW2CStatementRecords)
                        {
                            if (contract != null && !string.IsNullOrEmpty(contract.Ww2coEmployeeId) && !string.IsNullOrEmpty(contract.Ww2coCorrectionYear))
                            {
                                try
                                {
                                    ...
                                    {
                                        var correctionStatement = new TaxFormStatement3(contract.Ww2coEmployeeId, contract.Ww2coCorrectionYear, TaxFormTypes.FormW2C, contract.Recordkey, contract.WebW2cOnlineAdddate);
                            ...
                            if (contract != null && !string.IsNullOrEmpty(contract.Ww2oEmployeeId) && !string.IsNullOrEmpty(contract.Ww2oYear))
                            {
                                try
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(contract.Ww2oW2cFlag)
                                        ...
                                        && statements.Where(x => x.TaxYear == contract.Ww2oYear && x.TaxForm == TaxFormTypes.FormW2C).ToList().Count == 0)
                                    {
                                        // W-2's can only have a single correction.
                                        statements.RemoveAll(x => x.TaxYear == contract.Ww2oYear);
                                        var statement = new TaxFormStatement3(contract.Ww2oEmployeeId, contract.Ww2oYear, TaxFormTypes.FormW2, contract.Recordkey);
                                        statement.Notation = TaxFormNotations.Correction;
                                        statements.Add(statement);
                                    }
                                    else
                                    {
                                        if (statements.Where(x => (x.TaxForm == TaxFormTypes.FormW2 && x.Notation == TaxFormNotations.Correction)
                                            && x.TaxYear == contract.Ww2oYear).ToList().Count == 0)
                                        {
                                                ...
                                                || !string.IsNullOrEmpty(contract.Ww2oLocalNameC))
                                            {
                                                ...
                                                var overflowStatement = new TaxFormStatement3(contract.Ww2oEmployeeId, contract.Ww2oYear, TaxFormTypes.FormW2, contract.Recordkey);
                                                overflowStatement.Notation = TaxFormNotations.IsOverflow;
                                                statements.Add(overflowStatement);
                                            }
                                            else
                                            {
                                                statements.Add(new TaxFormStatement3(contract.Ww2oEmployeeId, contract.Ww2oYear, TaxFormTypes.FormW2, contract.Recordkey));
                                            }
                                        }
                                    }
                                }
                    ...
                    if (form1095cStatementRecords != null)
                    {
                        ...
                        foreach (var year in taxYears)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    var statement1095c = new TaxFormStatement3(mostRecentStatement.TfcwhHrperId, mostRecentStatement.TfcwhTaxYear, TaxFormTypes.Form1095C, mostRecentStatement.Recordkey);
                ...
                case TaxFormTypes.FormT4:
                    ...
                    if (formT4StatementRecords != null)
                    {
                        ...
                        foreach (var statement in formT4StatementRecordsList)
                        {
                            try
                            {
                                var firstStatement = new TaxFormStatement3(statement.Wt4oEmployeeId, statement.Wt4oYear, TaxFormTypes.FormT4, statement.Recordkey);
                                var numberOfSlips = 0;
                                ...
                                {
                                    statements.Add(new TaxFormStatement3(statement.Wt4oEmployeeId, statement.Wt4oYear, TaxFormTypes.FormT4, statement.Recordkey) { Notation = TaxFormNotations.IsOverflow });
                                }
                            }
                            catch (Exception e)
                            {

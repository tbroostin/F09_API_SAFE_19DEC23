                ...
                case AnonymousGradingType.Section:
                    foreach (var scs in studentCourseSecRecords)
                    {
                        try
                        {
                            ...
                            if (!scs.ScsMidRandomId.HasValue)
                            {
                                logger.Info(string.Format("Random grading IDs are generated by section but STUDENT.COURSE.SEC record {0} does not have a Mid Term random grading ID therefore is going to use random Id of finals",
                                    scs.Recordkey));
                                scs.ScsMidRandomId = scs.ScsRandomId;
                            }
                            PreliminaryAnonymousGrade pag = new PreliminaryAnonymousGrade(scs.ScsRandomId.ToString(), scs.ScsMidRandomId.ToString(),  finalGradeId, scs.ScsCourseSection, scs.Recordkey, finalGradeExpirationDate);
                ...
                case AnonymousGradingType.Term:
                    foreach (var scs in studentCourseSecRecords)
                    {
                        try { 
                            ...
                            if (!sttr.SttrMidRandomId.HasValue)
                            {
                                logger.Info(string.Format("Random grading IDs are generated by term but STUDENT.TERMS record {0} does not have a midterm random grading ID therefore is going to use random Id of finals",
                                    sttr.Recordkey));
                                sttr.SttrMidRandomId = sttr.SttrRandomId;
                            }
                            PreliminaryAnonymousGrade pag = new PreliminaryAnonymousGrade(sttr.SttrRandomId.ToString(), sttr.SttrMidRandomId.ToString(), finalGradeId, scs.ScsCourseSection, scs.Recordkey, finalGradeExpirationDate);
                            if (pag.CourseSectionId == sectionId)
                            {
                                sectionPreliminaryAnonymousGrading.AddAnonymousGradeForSection(pag);
                            }

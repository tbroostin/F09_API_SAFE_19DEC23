...
namespace Ellucian.Colleague.Domain.BudgetManagement.Entities
{
    ...
    public class SubtotalLineItem
    {
        ...
        /// <summary>
        /// Constructor that initializes a Subtotal Line Item entity.
        /// </summary>
        public SubtotalLineItem()
        {
            this.SubtotalBudgetComparables = new List<BudgetComparable>();
        }

        /// <summary>
        /// Method to add a budget comparable to a subtotal line item.
        /// </summary>
        /// <param name="subtotalBudgetComparable">A subtotal budget comparable.</param>
        public void AddBudgetComparable(BudgetComparable subtotalBudgetComparable)
        {
            if (subtotalBudgetComparable == null)
            {
                throw new ArgumentNullException("subtotalBudgetComparable", "The subtotal budget comparable cannot be null.");
            }

            // Subtotal budget comparables will have unique comparable numbers.
            if (!SubtotalBudgetComparables.Where(x => x.ComparableNumber == subtotalBudgetComparable.ComparableNumber).Any())
            {
                SubtotalBudgetComparables.Add(subtotalBudgetComparable);
            }
        }
    }
}

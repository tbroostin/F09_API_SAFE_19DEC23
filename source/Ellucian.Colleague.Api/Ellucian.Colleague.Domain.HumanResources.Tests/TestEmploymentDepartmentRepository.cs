...
{
    public class TestEmploymentDepartmentRepository
    {
        private string[,] employmentDepartments = {
        ...
        public IEnumerable<EmploymentDepartment> GetEmploymentDepartments()
        {
            var employmentDepartmentList = new List<EmploymentDepartment>();

            // There are 3 fields for each deduction type in the array
            var items = employmentDepartments.Length / 3;

            for (int x = 0; x < items; x++)
            {
                employmentDepartmentList.Add(new EmploymentDepartment(employmentDepartments[x, 0], employmentDepartments[x, 1], employmentDepartments[x, 2]));
            }
            return employmentDepartmentList;
        }
    }
}

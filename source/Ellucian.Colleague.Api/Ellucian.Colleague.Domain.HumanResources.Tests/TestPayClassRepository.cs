...
{
    public class TestPayClassRepository
    {
        private readonly string[,] _payClasses = {
        ...
        {
            var payClassList = new List<PayClass>();

            // There are 3 fields for each pay class in the array
            var items = _payClasses.Length / 3;

            for (var x = 0; x < items; x++)
            {
                payClassList.Add(new PayClass(_payClasses[x, 0], _payClasses[x, 1], _payClasses[x, 2]));
            }
            return payClassList;
        }
    }

...
namespace Ellucian.Colleague.Domain.Student.Tests
{
    public class TestCourseTypeRepository
    {
                private string[,] courseTypes = {
                                            //GUID                                    CODE    DESCRIPTION       SPECIAL_PROCESSING_2
                                            {"f660ba1c-e9fa-48a9-9948-a11bbd60ef03", "HONOR", "Honors",               "Y"}, 
        ...
        public IEnumerable<CourseType> Get()
        {
            ...
            var items = courseTypes.Length / 4;

            for (int x = 0; x < items; x++)
            {
                crsTypes.Add(new CourseType(courseTypes[x, 0], courseTypes[x, 1], courseTypes[x, 2], courseTypes[x, 3] != "N" ));
            }
            return crsTypes;
        }
    }
